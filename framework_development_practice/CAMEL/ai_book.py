...
#这个初始发言通常是一个开场白或第一个请求，旨在向助理角色（心理学家）提出具体的任务要求，引导整个对话走向。
input_msg = role_play_session.init_chat()

while n < chat_turn_limit:
    n += 1
    # step() 方法驱动一轮完整的对话，AI 用户和 AI 助理各发言一次
    # 执行一轮完整的对话：先由用户角色（作家）根据input_msg回复，
    # 然后助理角色（心理学家）根据用户的回复再回复。
    # step()返回两个响应对象：assistant_response（心理学家）和user_response（作家）
